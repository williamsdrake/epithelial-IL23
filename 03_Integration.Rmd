...
## Load datasets
```{r load datasets}
combined <- readRDS(file = "~/updatedAtlas-hu/04_data_objects/02_QC_objects/2023-04-19_02-QCOutput.RDS")
```

## Downsample objects (not run)

The purpose of this step is to compare treatment groups that start with different numbers of cells.

```{r downsample, eval = F}
source("~/updatedAtlas-hu/01_scripts/analysisHelpers.R")
cellNum <- c(ncol(health), ncol(perio), ncol(lad))
indx <- which.min(cellNum)

health <- downsampleSeurat(health, cellNum[indx])
perio <- downsampleSeurat(perio, cellNum[indx])
lad <- downsampleSeurat(lad, cellNum[indx])
```
...
combined_bySample <- lapply(X = combined_bySample, FUN = function(x) {
    x <- NormalizeData(x, verbose = FALSE)
    x <- FindVariableFeatures(x, verbose = FALSE)
    })

features <- SelectIntegrationFeatures(object.list = combined_bySample)
combined_bySample <- lapply(X = combined_bySample, FUN = function(x) {
...
anchors <- FindIntegrationAnchors(object.list = combined_bySample, reference = c(1:5), reduction = "rpca",
    dims = 1:50)
