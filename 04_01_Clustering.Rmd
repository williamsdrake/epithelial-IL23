...
oralIntegrated <- readRDS(file = paste0(baseDir, "2023-06-02_03-IntegrationOutput.RDS"))
```
...
oralIntegrated <- RunPCA(oralIntegrated, verbose = T)
oralIntegrated <- FindNeighbors(oralIntegrated, dims = 1:50)
oralIntegrated <- FindClusters(oralIntegrated, verbose = T, resolution = 0.2)
...
unique(oralIntegrated$disease)
Stacked_VlnPlot(seurat_object = oralIntegrated, 
                features = "IL23A", 
                group.by = "ref", 
                split.by = "disease", 
                x_lab_rotate = TRUE)
DimPlot_scCustom(seurat_object = oralIntegrated, split.by = "ref", label = T)
